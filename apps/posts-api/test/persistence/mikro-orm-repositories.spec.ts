...
describe('adapters do MikroORM', () => {
  ...
  describe('MikroOrmPostRepository', () => {
    ...
    it('findById devolve null para um id que não existe', async () => {
      expect(
        await inContext(() => posts.findById(PostId.generate())),
      ).toBeNull();
    });
    ...
    it('restaurar um post que nunca foi apagado não muda nada', async () => {
      ...
      expect(
        (await inContext(() => posts.findById(post.id)))?.isDeleted(),
      ).toBe(false);
    });

    it('findAll pagina por cursor em ordem de criação', async () => {
      const first = await givenAPost(module, {
        title: 'primeiro',
        createdAt: T0,
      });
      ...
      const next = await inContext(() =>
        posts.findAll({ first: 1, after: page.endCursor }),
      );

      expect(next.items.map((each) => each.id.value)).toEqual([
        second.id.value,
      ]);
    ...
    it('findAll com after nulo é a primeira página', async () => {
      ...
      const page = await inContext(() =>
        posts.findAll({ first: 10, after: null }),
      );
  ...
  describe('MikroOrmTagRepository', () => {
    it('acha pelo id e pelo nome, que é a chave única de negócio', async () => {
      ...
      expect((await inContext(() => tags.findById(tag.id)))?.name.value).toBe(
        'dev',
      );
      expect(
        (
          await inContext(() => tags.findByName(TagName.parse('dev')))
        )?.id.equals(tag.id),
      ).toBe(true);
    });
    ...
    it('devolve null quando não existe, em vez de estourar', async () => {
      ...
      expect(
        await inContext(() => tags.findByName(TagName.parse('inexistente'))),
      ).toBeNull();
    });
  });
  ...
  describe('MikroOrmUserRepository', () => {
    it('findById devolve o user com os papéis que ele carrega', async () => {
      ...
      expect(
        await inContext(() => users.findById(UserId.generate())),
      ).toBeNull();
    });
    ...
    it('findByEmail acha quem está ativo, e devolve null para um email desconhecido', async () => {
      ...
      expect(
        (await inContext(() => users.findByEmail(author.email)))?.id.equals(
          author.id,
        ),
      ).toBe(true);
      expect(
        await inContext(() =>
          users.findByEmail(Email.parse('outro@example.com')),
        ),
      ).toBeNull();
    });
    ...
    it('saveAll grava vários numa transação só', async () => {
      ...
      await inContext(async () => {
        const carregados = await Promise.all([
          users.findById(primeiro.id),
          users.findById(segundo.id),
        ]);
        await users.saveAll(
          carregados.filter((each): each is User => each !== null),
        );
      });
