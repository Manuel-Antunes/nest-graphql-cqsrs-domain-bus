...
describe('UserQueryResolver', () => {
  ...
  const aUser = (roles: readonly string[]): User =>
    User.register(
      UserId.generate(),
      { email: `u+${UserId.generate()}@example.com`, name: 'manuel' },
      roles,
      now,
    );
  ...
  const viewOf = (user: User) => {
    ...
    return user.hasRole(AUTHOR_ROLE)
      ? new AuthorView(state)
      : new UserView(state);
  };
  ...
  describe('__resolveType', () => {
    it('translates the view class into the schema type name', () => {
      expect(resolver.__resolveType(viewOf(aUser([AUTHOR_ROLE])))).toBe(
        'Author',
      );
    ...
    it('answers with the schema name, not the class name', () => {
      const names = [aUser([AUTHOR_ROLE]), aUser([])].map((user) =>
