...
import { givenAPost, givenATag, givenTheDefaultTag } from '../../../../test/support/post-fixtures';
...
describe('CompletePostCommand.Handler', () => {
  ...
  const complete = (postId: PostId, tagId: TagId) =>
    inRequestContext(module, () =>
      commands.execute(new CompletePostCommand.CompletePost(postId, tagId), new PostRequest(postId)),
    );
  ...
  it('carries the request through, so the chain stays one request', async () => {
    ...
    expect(PostRequest.of(events.ofType(PostCreatedEvent)[0])?.postId.equals(post.id)).toBe(true);
  });
  ...
  it('refuses a post that does not exist', async () => {
    ...
    await expect(complete(PostId.generate(), tag.id)).rejects.toThrow(PostNotFoundException);
  });

  it('refuses a tag that does not exist, so the post is not completed with a dangling reference', async () => {
    const post = await givenAPost(module);

    await expect(complete(post.id, TagId.generate())).rejects.toThrow(TagNotFoundException);

    expect((await freshEm(module).findOneOrFail(Post, { id: post.id })).isComplete()).toBe(false);
  });
