...
describe('OnPostUpdatedSubscription.OnPostUpdated', () => {
  ...
  it('without a postId, every updated post is of interest', () => {
    ...
    expect(
      new OnPostUpdatedSubscription.OnPostUpdated({ postId: null }).match(
        event('p1'),
      ),
    ).toBe(true);
  });
  ...
  it('is the same request — the same key — when the criteria match, and not when they differ', () => {
    expect(
      new OnPostUpdatedSubscription.OnPostUpdated({ postId: 'p1' }).key,
    ).toBe(new OnPostUpdatedSubscription.OnPostUpdated({ postId: 'p1' }).key);
    expect(
      new OnPostUpdatedSubscription.OnPostUpdated({ postId: 'p1' }).key,
    ).not.toBe(
      new OnPostUpdatedSubscription.OnPostUpdated({ postId: 'p2' }).key,
    );
    expect(new OnPostUpdatedSubscription.OnPostUpdated({}).key).not.toBe(
      new OnPostUpdatedSubscription.OnPostUpdated({ postId: 'p1' }).key,
    );
  });
});
