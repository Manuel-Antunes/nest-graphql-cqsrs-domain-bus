...
describe('FindAllPostsQuery.Handler', () => {
  ...
  it('returns exactly the requested page and flags that there is more', async () => {
    const page = await inRequestContext(module, () =>
      handler.execute(new FindAllPostsQuery.FindAllPosts(2)),
    );

    expect(page.items.map((p) => p.title.value)).toEqual([
      'primeiro',
      'segundo',
    ]);
  ...
  it('a cursor points at the last seen row so the next page starts after it', async () => {
    const first = await inRequestContext(module, () =>
      handler.execute(new FindAllPostsQuery.FindAllPosts(2)),
    );

    const second = await inRequestContext(module, () =>
      handler.execute(new FindAllPostsQuery.FindAllPosts(2, first.endCursor)),
    );
  ...
  it('the last full page knows there is nothing after it', async () => {
    const page = await inRequestContext(module, () =>
      handler.execute(new FindAllPostsQuery.FindAllPosts(3)),
    );
  ...
  it('the cursor of each edge is the cursor of that row', async () => {
    const all = await inRequestContext(module, () =>
      handler.execute(new FindAllPostsQuery.FindAllPosts(3)),
    );

    const afterFirst = await inRequestContext(module, () =>
      handler.execute(
        new FindAllPostsQuery.FindAllPosts(3, all.from(all.items[0])),
      ),
    );

    expect(afterFirst.items.map((p) => p.title.value)).toEqual([
      'segundo',
      'terceiro',
    ]);
  });
  ...
  it('without a first the page size is the default, and it is capped at the maximum', () => {
    ...
    expect(new FindAllPostsQuery.FindAllPosts(10_000).first).toBe(
