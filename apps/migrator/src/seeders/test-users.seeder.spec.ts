...
import { bootstrap, type MigratorContext } from '../app/bootstrap';
import { PostsMigratorModule } from '../app/posts.module';
import { withSeederContainer } from './container';
import { SEED_PASSWORD, TestUsersSeeder, seededUsers } from './test-users.seeder';

describe('seeding the users the system should have', () => {
  let context: MigratorContext;

  const runSeeder = () =>
    withSeederContainer(context.app, () => context.orm.seeder.seed(TestUsersSeeder));

  const credentials = () =>
    context.orm.em.fork().getConnection().execute<{ email: string; provider_id: string; role: string | null; password: string | null }[]>(
  ...
  it('creates every user it declares, each with a credential account', async () => {
    ...
    expect(rows.map(row => row.email)).toEqual(seededUsers().map(user => user.email).sort());
  ...
  it('gives the author its role, which is what a write is authorised against', async () => {
    ...
    expect(rows.find(row => row.email === 'manuel@example.com')?.role).toBe('author');
    expect(rows.find(row => row.email === 'promovido@example.com')?.role).toBe('author');
    expect(rows.find(row => row.email === 'leitor@example.com')?.role).not.toBe('author');
  });
  ...
  it('seeds a credential that actually signs in, which inserting rows would not', async () => {
    ...
    const signedIn = await inRequestContext(context.orm, () =>
      auth.api.signInEmail({ body: { email: 'manuel@example.com', password: SEED_PASSWORD } }),
    );
