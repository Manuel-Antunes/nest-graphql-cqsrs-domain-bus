...
describe('CreatePostCommand.Handler', () => {
  ...
  const execute = (command: CreatePostCommand.CreatePost) => commands.execute(command, new PostRequest(command.postId));
  ...
  it('publishes PostCreated and returns the id', async () => {
    ...
    const result = await execute(new CreatePostCommand.CreatePost(id, ' Nest + GraphQL ', 'oi', 'manuel'));
  ...
  it('saves the post within the command', async () => {
    const id = newPostId();
    await execute(new CreatePostCommand.CreatePost(id, 'Nest + GraphQL', 'oi', 'manuel'));

    const saved = await freshEm(module).findOneOrFail(Post, { id }, { populate: ['tags'] });

    expect(saved).toMatchObject({ id, title: 'Nest + GraphQL', content: 'oi', author: 'manuel', version: 1 });
  ...
  it('rejects a blank title without saving anything', async () => {
    ...
    await expect(execute(new CreatePostCommand.CreatePost(id, '   ', 'oi', 'manuel'))).rejects.toThrow(InvalidPostException);
  ...
  it('rejects an id that already exists', async () => {
    ...
    await expect(execute(new CreatePostCommand.CreatePost(existing.id, 'outro', 'oi', 'manuel'))).rejects.toThrow(
