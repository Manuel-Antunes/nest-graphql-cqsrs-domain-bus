...
a `@Publisher` with no namespace (nothing would ever be routed to it), one whose `client` is not a
`ClientProxy`, and a client no `ChannelAddressing` serves (the message would go out with no routing
key, and a topic exchange drops what matches no binding without a line in the log).

On RabbitMQ the routing key is `namespace.Name.aggregateTag` — three segments, because a topic
...
- `outbox-routing.spec` — selection by namespace: the destination that takes it, the namespace nobody
  ...
  (fan-out), `EVERY_NAMESPACE`, and **the three things the table refuses to do quietly** — a publisher
  with no namespace, a `client` that is not a `ClientProxy`, and a client no addressing serves. Each
  refusal names the offending class.
