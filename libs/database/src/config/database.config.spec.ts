import { DEFAULT_POSTGRES_URL, databaseConfig, postgresDatabase, postgresUrl } from './database.config';
...
describe('postgresDatabase', () => {
  ...
  it('puts the service in a schema of its own on the shared connection', () => {
    expect(postgresDatabase('posts')).toMatchObject({ schema: 'posts', clientUrl: postgresUrl() });
  ...
  it('leaves the schema to the migrations, and only makes sure the database is there', () => {
    expect(postgresDatabase('posts')).toMatchObject({ ensureDatabase: { create: false } });
  });
  ...
  it('lets the caller override anything it decided', () => {
    const config = postgresDatabase('posts', { ensureDatabase: false, allowGlobalContext: true });

    expect(config).toMatchObject({ ensureDatabase: false, allowGlobalContext: true });
  });
