...
import { TRANSPORT_IDENTIFIER, TRANSPORT_MESSAGE_TYPE, TRANSPORT_ORIGIN } from '../outbound/event-envelope';
import { CORRELATION_ID, CorrelatedRequestContext, TransportRequestContext } from '../request-context';
...
const rpcContext = (metadata: Record<string, string>): ExecutionContext =>
  ({
    ...
    switchToRpc: () => ({ getData: () => envelope(metadata), getContext: () => ({}) }),
...
describe('the tenant of a message', () => {
  const resolver = new TransportTenantResolver(new IncomingRequest(new CorrelatedRequestContext()));
  ...
  describe('read off the envelope the transport delivered', () => {
    it('answers with the tenant the publishing service put on the request', () => {
      expect(resolver.tenantOf(rpcContext({ [TENANT_HEADER]: 'Acme' }))).toBe('acme');
    });
    ...
    it('normalizes the word a producer interpolated in place of a missing tenant', () => {
      expect(resolver.tenantOf(rpcContext({ [TENANT_HEADER]: 'undefined' }))).toBe(ROOT_TENANT);
    });
  });

  describe('the same resolver still answers for http, because one service serves both', () => {
    it('falls back to the header', () => {
      expect(resolver.tenantOf(httpContext({ [TENANT_HEADER]: 'globex' }))).toBe('globex');
    });
  });
  ...
  describe('reading a tenant off whatever context class the application rebuilt', () => {
    it('reads the attributes of the generic transport context', () => {
      const context = new TransportRequestContext('c-1', 'm-1', { [TENANT_HEADER]: 'acme' });
    ...
    it('reads the attributes an application context declares for the wire', () => {
      ...
      expect(TransportTenantResolver.tenantCarriedBy(new PostRequest())).toBe('initech');
    });

    it('answers nothing for a context that carries no tenant, and for none at all', () => {
      expect(TransportTenantResolver.tenantCarriedBy(new TransportRequestContext('c-1'))).toBeUndefined();
      expect(TransportTenantResolver.tenantCarriedBy(new AsyncContext())).toBeUndefined();
      expect(TransportTenantResolver.tenantCarriedBy(undefined)).toBeUndefined();
    });
  });
});
