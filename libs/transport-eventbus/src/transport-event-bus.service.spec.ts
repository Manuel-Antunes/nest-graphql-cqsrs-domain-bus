...
import { SinglePatternAddressing } from './outbound/addressing/single-pattern.addressing';
...
describe('the transport event bus (the vendored base)', () => {
  ...
  describe('the pattern an event goes out under', () => {
    it('is the single pattern, which is the mode this service publishes in', async () => {
      await eventBus.publish(new RabbitWithDefEvent('RabbitWithDefEvent'));

      expect(rabbit.patterns()).toEqual(['TRANSPORT_EVENT_BUS_PATTERN']);
    });
