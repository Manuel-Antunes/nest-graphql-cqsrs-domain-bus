 ...
 * the payload. That still works here — it is what {@link ChannelAddressing} returns unless the
 * transport addresses better — and the environment variable is upstream's too, so a deployment can
 * rename the pattern without touching code.
 *
 * On RabbitMQ with `wildcards: true` the pattern **is** the routing key, and
 * {@link RabbitMqAddressing} builds one per event instead, so a consumer can bind to
