...
import { Addressing, type ChannelAddressing } from './addressing/channel-addressing';
import type { EventAddress } from './event-address';
import type { EventEnvelope } from './event-envelope';

/** A destination already resolved: what it is called, what it takes, its client and its addressing. */
export class Route {
  ...
  send(envelope: EventEnvelope<object>, address: EventAddress): Observable<void> {
    ...
    return this.client.emit(this.addressing.pattern(address), envelope).pipe(map(() => undefined));
  }
}
 ...
 *
 * ## The three things this refuses to do quietly
 ...
 * 3. **a client no {@link ChannelAddressing} serves** — the message would go out with no routing key,
 *    and a topic exchange drops what matches no binding without saying anything.
 *
 * All three bring the table's resolution down, naming the class.
 *
...
export class OutboxRouting {
  ...
  /**
   * The addressings, specific ones first. A catch-all answers for every client, so consulting it
   * before {@link RabbitMqAddressing} would mean a RabbitMQ destination silently losing its routing
   * keys — and the order providers happen to be registered in is not something a topology should
   * depend on.
   */
  private addressings(): ChannelAddressing[] {
    return this.discovery
      .getProviders({ metadataKey: Addressing.KEY })
      .map((wrapper) => wrapper.instance as ChannelAddressing)
      .filter((addressing): addressing is ChannelAddressing => typeof addressing?.serves === 'function')
      .sort((one, other) => Number(one.catchAll ?? false) - Number(other.catchAll ?? false));
  }
  ...
  private resolve(): Route[] {
    return this.discovery.getProviders({ metadataKey: Publisher.KEY }).map((wrapper) => {
      ...
      const addressings = this.addressings();
      const addressing = addressings.find((candidate) => candidate.serves(client));
      if (!addressing) {
        throw new Error(
          `no ChannelAddressing serves the client of ${declaration} (${client.constructor.name}). ` +
            `Known: ${addressings.map((known) => String(known.transport)).join(', ') || '(none)'}. ` +
            `Without addressing the message goes out with no routing key and the exchange drops it ` +
