...
export class EventSourcedRepository<T extends EventSourced> {
  ...
  static of<T extends EventSourced>(
    ...
    return {
      provide: token,
      inject: [EventStore],
      useFactory: (store: EventStore) => new EventSourcedRepository(aggregate, store),
    };
  }
  ...
  async load(id: StreamIdentity): Promise<T | null> {
    const history = await this.store.read(streamIdOf(id));
    if (history.length === 0) {
      return null;
    }
  ...
  save(aggregate: T): Promise<void> {
    return this.store.append(aggregate.id.value, aggregate.getUncommittedEvents());
  }
}
