...
- **`DiscoveryModule` is Nest's own**, and it is what finds the `@Publisher` and `@Addressing`
  classes wherever they are declared.
...
With `RabbitMqAddressing` (in `transportEventBusProviders`), the pattern is three segments:
...
All three bring the table down on the first publish, naming the class:
...
| a client no `ChannelAddressing` serves | a message with no routing key, dropped by the exchange without a line in the log |
...
@Module({
  ...
  providers: [
    ...
    { provide: RequestContextCodec, useClass: CorrelatedRequestContext },
    SinglePatternAddressing,          // everything under one pattern
    RabbitPublisher,
  ],
})
export class TransportModule {}
...
`SinglePatternAddressing` serves any client, so it is opt-in: being addressed wrongly is worth failing
over, and an addressing that quietly answered for every transport would put that failure back. A
class that implements `publish` keeps upstream's behaviour entirely — the bus calls it instead of
emitting on the client.
...
addressing and the serializer are exercised as they are in production) and it matches patterns
...
const tagging = await startInProcessService(
  await Test.createTestingModule({ imports: [AppModule], providers: [RecordingAddressing] })
...
`RecordingClient` keeps what it was asked to send instead of sending it, and `RecordingAddressing`
gives it the same topic pattern a real transport would get. It is declared by the spec that needs it,
which is what keeps a double from quietly answering for a client that should have had a real
addressing.
...
Three small classes, and each says one thing:

```ts
@Injectable()
@Addressing()
export class KafkaAddressing implements ChannelAddressing {   // where it goes
  ...
  pattern(address: EventAddress): string {
    return address.qualifiedName;              // the topic
  }
}
...
export class KafkaEventEnvelopeSerializer extends EventEnvelopeSerializer {   // how it is written
  protected serializeEnvelope(envelope: EventEnvelope<Record<string, unknown>>, packet: ReadPacket) {
    return { pattern: packet.pattern, data: { key: envelope.metadata[TRANSPORT_TAGS], value: envelope.data, headers: envelope.metadata } };
  }
}
...
Declare the addressing as a provider and the routing table picks it for the destinations whose client
it serves; name the other two in that transport's options. A transport that addresses the way RabbitMQ
does needs even less: extend `TopicAddressing` and say which client it recognises.

Nothing else changes — not the forwarder, not the envelope, not the other destinations. The forwarder
emits an `EventEnvelope` and never learns a protocol.
...
`EventEnvelopeFactory`, `RabbitMqAddressing`, `MemoryAddressing`);
...
| `@Addressing()` | marks a `ChannelAddressing` |
...
| `EventEnvelope` / `EventAddress` | the wire format and what it is addressed by |
| `TopicAddressing` / `RabbitMqAddressing` / `MemoryAddressing` / `SinglePatternAddressing` | how each transport addresses |
| `MemoryClient` / `startInProcessService` / `RecordingClient` / `RecordingAddressing` | the doubles |
