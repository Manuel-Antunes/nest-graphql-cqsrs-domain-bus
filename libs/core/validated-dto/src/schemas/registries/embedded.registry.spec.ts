...
import {
  ...
  EMBEDDED_REGISTRY,
  getEmbedded,
} from './embedded.registry';

describe('EMBEDDED_REGISTRY', () => {
  class Codigo extends ValidatedDto.Scalar(
    z.string().min(1).brand<'Codigo'>(),
  ) {}
  ...
  it('um registry isolado cai no global para o que ele não conhece', () => {
    ...
    expect(getEmbedded(doGlobal, isolado)).toEqual({
      kind: 'scalar',
      target: Codigo,
    });
  });
  ...
  it('o que está no registry isolado é lido de lá, sem passar pelo global', () => {
    ...
    expect(getEmbedded(schema, isolado)).toEqual({
      kind: 'object',
      target: Codigo,
    });
  ...
  it('o registry global é o padrão de leitura', () => {
    expect(getEmbedded(Codigo.field(), EMBEDDED_REGISTRY)).toEqual(
      getEmbedded(Codigo.field()),
    );
  });
});
