...
import type { ButtonProps } from '@nestposts/ui/components/ui/button';
  ...
  const errors = errorCodes.map((error) => {
    switch (error) {
      case 'file-invalid-type':
        return `only ${Object.values(limits.accept ?? {})
          .flat()
          .join(', ')} are allowed`;
      case 'file-too-large':
        return `max size is ${
  ...
  const dropzone = rootUseDropzone({
    ...
    onDropAccepted: async (newFiles) => {
      ...
      const onDropFilePromises = slicedNewFiles.map(async (file, index) => {
        ...
        if (fileCount + 1 > maxNewFiles && fileStatus && fileStatus.id) {
  ...
  ({ className, children, ...props }, forwardedRef) => {
    ...
    return (
      <div
        ...
        aria-label="dropzone"
        className={cn(
          'flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 shadow-xs ring-offset-background transition-[color,box-shadow] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
...
const DropzoneRemoveFile: React.ForwardRefExoticComponent<
  DropzoneRemoveFileProps & React.RefAttributes<HTMLButtonElement>
> = forwardRef<HTMLButtonElement, DropzoneRemoveFileProps>(
  ({ className, ...props }, ref) => {
    const context = useDropzoneFileListContext();
    if (!context) {
      throw new Error(
...
const DropzoneRetryFile: React.ForwardRefExoticComponent<
  DropzoneRetryFileProps & React.RefAttributes<HTMLButtonElement>
> = forwardRef<HTMLButtonElement, DropzoneRetryFileProps>(
  ({ className, ...props }, ref) => {
    const context = useDropzoneFileListContext();
