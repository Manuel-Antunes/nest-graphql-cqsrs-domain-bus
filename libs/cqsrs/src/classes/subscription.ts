...
import { EVENT_TYPE_SYMBOL } from '../interfaces/constants';
import type { ISubscription } from '../interfaces/subscription.interface';

/** O tipo do evento que uma subscription entrega. */
export type SubscriptionEvent<S> = S extends ISubscription<infer TEvent> ? TEvent : never;
 ...
 *
 * ## Esta classe é um helper; o contrato é a interface
 * Quem define o que é uma subscription é a {@link ISubscription}: `filter`, `key` e o tipo do evento.
 * O `SubscriptionBus` depende dela, não desta classe.
 *
 * Esta classe é a **implementação conveniente** do contrato — a que você vai querer na maior parte
 * das vezes, porque acrescenta a ideia de *critério* e resolve as três exigências a partir dele: a
 * chave sai do critério serializado, o filtro padrão passa tudo, e o fantasma do tipo vem de graça.
 * Quem precisar de outra coisa implementa a interface direto, sem herdar nada.
 *
 ...
 * critério pedem, literalmente, a mesma coisa — então recebem o mesmo `Observable`, e a fonte
 ...
 */
export abstract class Subscription<TEvent, TCriteria = void> implements ISubscription<TEvent> {
