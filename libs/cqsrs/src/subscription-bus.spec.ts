...
import { EVENT_TYPE_SYMBOL, type ISubscription, type ISubscriptionHandler } from './interfaces';
...
/**
 * Uma subscription que **não herda** de `Subscription`: só cumpre o contrato da `ISubscription`.
 * É o caminho agnóstico — o bus depende da interface, não da hierarquia.
 */
class OnEvenCounters implements ISubscription<CounterEvent> {
  readonly [EVENT_TYPE_SYMBOL]: CounterEvent;
  readonly key = 'OnEvenCounters()';

  filter(event: CounterEvent): boolean {
    return event.value % 2 === 0;
  }
}

@SubscriptionHandler(OnEvenCounters)
class OnEvenCountersHandler implements ISubscriptionHandler<OnEvenCounters> {
...
describe('SubscriptionBus', () => {
  ...
  beforeAll(async () => {
    module = await Test.createTestingModule({
      ...
      providers: [OnCounterSubscriptionHandler, OnEvenCountersHandler],
    }).compile();
    await module.init();
  ...
  it('serves a subscription that only implements ISubscription, without extending Subscription', () => {
    const evens = collect(bus.subscribe(new OnEvenCounters()));

    eventBus.publish(new CounterEvent('a', 1));
    eventBus.publish(new CounterEvent('a', 2));
    eventBus.publish(new CounterEvent('a', 3));
    eventBus.publish(new CounterEvent('a', 4));

    expect(values(evens)).toEqual([2, 4]);
  });

  it('shares the stream of an interface-only subscription by its key, like any other', () => {
    const before = upstream();
    const first = bus.subscribe(new OnEvenCounters());
    const second = bus.subscribe(new OnEvenCounters());
