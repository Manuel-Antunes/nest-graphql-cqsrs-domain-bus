 ...
 *    por stream, não por assinante. Ele conhece a {@link ISubscription}, não a classe `Subscription`:
 *    qualquer coisa que responda `filter` e `key` serve;
 ...
 */
@Injectable()
export class SubscriptionBus<SubscriptionBase extends ISubscription = ISubscription> implements ISubscriptionBus {
  ...
  subscribe<TEvent>(subscription: ISubscription<TEvent>, asyncContext?: AsyncContext): Observable<TEvent> {
    ...
    const key = `${subscriptionId}:${subscription.key}`;
    ...
    const stream: Observable<TEvent> = defer(() => {
      ...
      filter((event) => subscription.filter(event)),
