import { Injectable, type InjectableOptions, type Type } from '@nestjs/common';
 ...
 *
 * @param subscription A *classe* da subscription tratada por este handler — a metadata do id vai nela.
 * @param options Opções repassadas ao `@Injectable` (escopo, por exemplo).
 */
export const SubscriptionHandler = (subscription: Type<ISubscription>, options?: InjectableOptions): ClassDecorator => {
  return (target) => {
    if (!Reflect.hasOwnMetadata(SUBSCRIPTION_METADATA, subscription)) {
      Reflect.defineMetadata(SUBSCRIPTION_METADATA, { id: randomUUID() }, subscription);
    }
